...
export const SendMessageMoment = async (message, selectedMomentId, uid, isGhost = false) => {
  try {
    const body = {
      data: {
        ...
        analytics: {
          amplitude: {
            device_id: isGhost ? generateUUIDv4Upper() : generateUUIDv4Upper(), // Trong app thực tế nên lưu cố định nếu không ẩn danh
            session_id: -1,
          },
          google_analytics: {
            app_instance_id: isGhost ? generateUUIDv4Upper().replace(/-/g, '') : "e88d4daed0ded172248753851bf67772",
          },
    ...
    if (isGhost) {
      // Có thể thêm một vài thông số fake khác nếu cần để bypass
    }
