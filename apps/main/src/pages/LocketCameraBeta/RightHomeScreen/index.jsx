...
import { useAuthStore, useMessagesStore, useReadReceipts } from "@/stores";
...
const RightHomeScreen = ({ setIsHomeOpen }) => {
  ...
  const { sendReadReceipts } = useReadReceipts();
  ...
  const handleSelectChat = async (chat) => {
    ...
    if (chat.isRead === false && sendReadReceipts) {
  ...
  return (
    <>
      {/* ================= Conversation list ================= */}
      <div
        className={`fixed inset-0 transition-transform duration-500 bg-base-100 overflow-hidden
        ${
          isHomeOpen
            ? selectedChat
              ? "-translate-x-full"
