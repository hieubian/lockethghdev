...
export default function SettingAppIcon() {
  ...
  return (
    <div className="bg-base-100 rounded-2xl shadow-xl mb-8">
      <h2 className="text-2xl font-bold text-base-content flex items-center gap-2 p-5 pb-0">
          ...
          {ICONS.map((icon) => {
            ...
            return (
              <button
                ...
                className={`
                    relative p-1 rounded-xl transition-transform duration-300
                    ${isPWA || loading ? "opacity-50 cursor-not-allowed" : ""}
                    ${
                      active
                        ? "ring-4 ring-primary scale-105 shadow-lg"
                        : "border border-base-300 hover:scale-105"
                    }
                  `}
              >
                {active && (
